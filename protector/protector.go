...
func (p Protector) GenSessionKey(sessionKey string) string {
	if p.Hash == "" {
		panic("Hash is empty")
	}
	...
	for i := 0; i < len(p.Hash); i++ {
		...
		var hash, err = strconv.Atoi(p.calcHash(sessionKey, digit))
		if err != nil {
			panic(err)
		}
		result += hash
...
func (p Protector) calcHash(sessionKey string, value int) string {
	var sb strings.Builder
	switch value {
	case 1:
		sb.WriteString("00")
